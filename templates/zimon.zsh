...
function zimon() {
  ...
  if [[ $cmd == "init" ]]; then
    ...
    # setup colors
    local oldzshrc=$(cat ~/.zshrc)
    local setupcolors=$(cat ~/zfuncs/templates/setup-template-colors.sh)
    if [[ $oldzshrc != *"$setupcolors"* ]]; then
      echo "" >> ~/.zshrc
      cat ~/zfuncs/templates/setup-template-colors.sh >> ~/.zshrc
      source ~/.zshrc
    else
    ...
    local setuptemplate=$(cat ~/zfuncs/templates/setup-template.sh)
    local oldzshrc=$(cat ~/.zshrc)
    ...
    if [[ $flags == *"--no-cred"* ]]; then
      ...
      local currentpassword=$(echo $oldzshrc | rg '(stsauth authenticate)')
